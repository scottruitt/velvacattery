...
BEGIN
{
  ...
  $postmaster        = '';
  @referers          = qw(tankt.com velvacattery.com www.velvacattery.com);
  @allow_mail_to     = qw(ddoernberg@aol.com);
  @recipients        = ();
  %recipient_alias   = ();
...
sub send_main_email_header {
  ...
  my $subject = $self->{FormConfig}{subject} || 'New puppy request';
  if ($self->{CFG}{secure}) {
...
sub send_main_email_body_header {
  ...
  $self->mailer->print(<<END);
Diana, here is a new kitten request for you. It was submitted by
